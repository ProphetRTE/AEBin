...
New Parameters:
    ...
    -preciseTotals [t/f]: If true, turtle will write exactly what it mined to the logs. It may also transmit it over aenet.
...
aenetEnabled = false --Default aenet on or off  [Default false]
...
saveFile = "AE_Quarry_Restore" --Where it saves restore data [Default "AE_Quarry_Restore"]
...
legacyAENet = false --Use this if playing 1.4.7
...
-aenet: [t/f] If true and you have a wireless modem on the turtle, will attempt to make a aenet connection for sending important information to a screen
...
-sendChannel: [number] This is what channel your turtle will send aenet messages on
-receiveChannel: [number] This is what channel your turtle will receive aenet messages on
-legacyAENet: [t/f] Check true if using 1.4.7
...
-preciseTotals: [t/f] If true (and turtle.inspect exists), it will log a detailed record of every block the turtle mines and send it over aenet
...
Examples [1]:
  ...
  You just told it to start a quarry with dimensions 10x6x3, and "-default" means it won't prompt you about invert or aenet. Wasn't that easy?
Examples [2]:
  Okay, so you've got the basics of this now, so if you want, you can type in really long strings of stuff to make the quarry do exactly what you want. Now, say you want a 40x20x9, but you want it to go down to diamond level, and you're on the surface (at y = 64). You also want it to send aenet messages to your computer so you can see how its doing.
Examples [2] [cont.]:
  ...
  "quarry -dim 40x20x9 -invert false -startDown 45 -aenet true -enderChest 12 -restore"
  BAM. Now you can just let that turtle do it's thing
Tips:
  The order of the parameters doesn't matter. "quarry -invert false -aenet true" is the same as "quarry -aenet true -invert false"
...
if peripheral.find then
  supportsAENet = peripheral.find("modem") or false
else
  supportsAENet = (peripheral.getType("right") == "modem") or false
end
...
if not(turtle or tArgs["help"] or tArgs["-help"] or tArgs["-?"] or tArgs["?"]) then --If all of these are false then
  print("This is not a turtle, you might be looking for the \"Companion AENet Program\" \nCheck My forum thread for that")
...
--AENet
addParam("aenet", "AENet Enabled","boolean",true, supportsAENet, "aenetEnabled")
addParam("sendChannel", "AENet Send Channel", "number 1-65535", false, supportsAENet, "channels.send")
addParam("receiveChannel","AENet Receive Channel", "number 1-65535", false, supportsAENet, "channels.receive")
addParam("fingerprint","Sending Fingerprint", "string", false, supportsAENet, "channels.fingerprint")
addParam("legacyAENet","Legacy AENet","boolean", false, supportsAENet)
--Quad Rotor --Must be before GPS
if addParam("quad", "Quad Rotor Enabled","boolean",nil, aenetEnabled, "quadEnabled") then --This returns true if param found :3
  gpsEnabled = true
end
addParam("quadTimeout","Quad Rotor Timeout","number 1-1000000", nil, quadEnabled) --The amount of time to wait for a quadRotor
--GPS
addParam("gps", "GPS Location Services", "force", nil, (not restoreFoundSwitch) and supportsAENet and not quadEnabled, "gpsEnabled" ) --Has these triggers so that does not record position if restarted.
...
addParam("preciseTotals","Precise Totals","boolean", aenetEnabled and turtle.inspect, turtle.getItemDetail ~= nil)
...
--Made by AEV1ridis's Variable Size Quarry
...
if aenetEnabled then
  screen(1,1)
  print("AENet is Enabled")
  ...
  local i = 0
    repeat
      ...
      repeat
        ...
        if legacyAENet then --For that one guy that uses 1.4.7
...
function biometrics(isAtBedrock, requestQuad)
  if not aenetEnabled then return end --This function won't work if aenet not enabled :P
  ...
  repeat
    ...
    if legacyAENet and type(received) == "string" then
  ...
  if message == "stop" or message == "quit" or message == "kill" then
    count(true)
    display()
    error("AENet said to stop...",0)
  end
  if message == "return" then
    endingProcedure()
    error('AENet said go back to start...',0)
  end
...
function display() --This is just the last screen that displays at the end
  ...
  if aenetEnabled then
    ...
    if legacyAENet then --This was the traditional stopping signal
...
function logMiningRun(textExtension, extras) --Logging mining runs
  ...
  write("Was using aenet? ",boolToText(aenetEnabled))
...
function mine(doDigDown, doDigUp, outOfPath,doCheckInv) -- Basic Move Forward
  ...
  isInPath = (not outOfPath) --For aenet
...
function checkSanity()
  ...
  if xPos < 0 or xPos > x or zPos < 0 or zPos > z or yPos < 0 then
    ...
    print("If you could also run \"pastebin put AE_Quarry_Restore\" and give me that code it would be great")
    error("",0)]]
  end
end
...
function bedrock()
  ...
  if aenetEnabled then biometrics() end
