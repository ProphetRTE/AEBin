...
return {
  ...
  enabled = function() return settings.get("aes.shell.enabled") end,
  ...
  setup = function(path)
    ...
    function os.run(env, path, ...)
      ...
      local ok, err
      if settings.get("aes.shell.traceback") then
