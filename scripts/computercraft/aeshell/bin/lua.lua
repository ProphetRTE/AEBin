...
local function pretty(t, n)
  ...
  local fit_height = settings.get("aes.lua.pretty_height", true)
...
local function process_auto_run_file(folderPath, file)
  ...
  local ok, result
  if settings.get("aes.lua.traceback", true) then
...
local history_file = settings.get("aes.lua.history_file", ".lua_history")
...
while running do
  ...
  local line
  if readline and readline.read and settings.get("aes.lua.highlight") then
    line = readline.read {
      history = history,
  ...
  if line:find("%S") then
    if line ~= history[#history] then
      ...
      local max = tonumber(settings.get("aes.lua.history_max", 1e4)) or 1e4
