local history = {}
do
  local history_file = settings.get("aes.shell.history_file", ".shell_history")
  ...
  local max = tonumber(settings.get("aes.shell.history_max", 1e4)) or 1e4
  if #history > max then
...
local worker = coroutine.create(function()
  ...
  while running do
    local scrollback = tonumber(settings.get("aes.shell.scroll_max", 1e3))
